...
```
Average mpg tends to be higher for manual transmission with 95% of confidence.
...
The variables "hp", "qsec" and "carb" are significantly impacted (both methods), with p-value lower that 0.05. They are considered for a refined modelling.
...
par(mfrow = c(2, 2)); plot(fit_hp_carb) # the best
summary(fit_hp_carb)$coefficients
